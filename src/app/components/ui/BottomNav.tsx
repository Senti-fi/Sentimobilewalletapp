...
export default function BottomNav({ active, onChange }: BottomNavProps) {
  return (
    <div className="bg-[#0a142f] border-t border-[#1b2c4f] flex items-start px-5 pt-[14px] pb-[14px] gap-2 shrink-0 w-full">
      {TABS.map(({ id, label, Icon }) => {
        ...
        return (
          ...
          >
            <Icon
              className={`w-4 h-4 transition-colors ${isActive ? 'text-[#00f0ff]' : 'text-[#8ac7ff]'}`}
              strokeWidth={isActive ? 2.5 : 1.5}
            />
            <span
              className={`text-[13px] font-medium leading-[18px] whitespace-nowrap transition-colors ${
                isActive ? 'text-[#00f0ff]' : 'text-[#8ac7ff]'
              }`}
            >
              {label}
            </span>
